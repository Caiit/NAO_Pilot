...
- Sending large messages (images, move files)
- Importing NaoQi SDK did not work, so sockets are used
- Connection between java and python

## Defending decision
- Sockets zijn kut --> Softbank vragen om goede SDK
- SDK is much easier and better connection
- Advantage current solution: can be used on different kind of robot when creating robot script for that robot
